...
export const AddVehicle = () => {
    ...
    const kentekenRegex = /^(?:\d{2}-[A-Z]{2}-[A-Z]{2}|\d{2}-[A-Z]{3}-\d{1}|\d{1}-[A-Z]{3}-\d{2}|[A-Z]{2}-\d{3}-[A-Z]{1}|[A-Z]{1}-\d{3}-[A-Z]{2}|[A-Z]{3}-\d{2}-[A-Z]{1})$/i;
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        const updatedValue = name === "max_capaciteit"
            ? parseInt(value)
            : name === "kenteken"
        ...
        if (name === "kenteken") {
            ...
            setKentekenValid(formatted.trim() !== "" && kentekenRegex.test(formatted));
        }
    };
